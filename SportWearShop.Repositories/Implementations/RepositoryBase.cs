...
public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class
{
    ...
    public virtual async Task<(List<TEntity> Items, int TotalCount)> GetAllAsync(
            ...
            CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            List<TEntity> items = await query.ToListAsync(cancellationToken);
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error while querying {EntityName}",
                typeof(TEntity).Name);
